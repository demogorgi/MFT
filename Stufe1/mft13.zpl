...
subto convexCostsURatio:
      forall <i, k> in N * { 1 .. numIntervalsD }: cru[i] >= is_u[i] * ( slopeD[k] * ( x[i] - knotsD[k] ) + h(knotsD[k], aD, dD, eD) );
# Konvexe Kosten Ratioabweichung Kuerzung
subto convexCostsZRatio:
      forall <i, k> in N * { 1 .. numIntervalsD }: crz[i] >= is_z[i] * ( slopeD[k] * ( y[i] - knotsD[k] ) + h(knotsD[k], aD, dD, eD) );
